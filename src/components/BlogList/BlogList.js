                    ...
                    <div className={`col-span-8 md:col-span-12 ${props.blRight}`}>
                        {blogs.map((blog, bitem) => (
                            <div className={`post mb-[70px]  ${blog.blClass}`} key={bitem}>
                                ...
                                <h3 className="text-[34px] md:text-[25px] sm:text-[22px] text-[#282e3f] font-normal font-heading-font leading-[40px] transition-all mb-[20px] group"><Link className="group-hover:text-[#c0b596] transition-all" onClick={ClickHandler} to={`/blog-single/${blog.slug}`}>{blog.title}</Link></h3>
                                ...
                                <Link onClick={ClickHandler} to={`/blog-single/${blog.slug}`} className="text-[#666] uppercase underline font-base-font text-[15px] font-semibold transition-all hover:text-[#c0b596]">READ MORE...</Link>
                            </div>
                        ))}
                    </div>
